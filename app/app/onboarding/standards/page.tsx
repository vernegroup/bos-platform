...
export default async function StandardsPage(){
 ...
 return <>
  ...
  <section className="bos-standard-list bos-operational-list">
   ...
   {standards.map(s=><Link href={`/app/onboarding/standards/${s.id}`} className="bos-standard-list-row" key={s.id}><strong>{s.name}</strong><span>{s.area||"—"}</span><b>{s.currentVersion||"DRAFT"}</b><span>{s.taskCount}</span><span>{s.updatedAt||"—"}</span><em data-status={s.status}>{s.status}</em><i>→</i></Link>)}
