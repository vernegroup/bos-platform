...
async function decide(formData:FormData) {
  ...
  if(decision==="READY"&&!formalitiesConfirmed){
    const query=new URLSearchParams({formalitiesError:"1",summary,recommendations});
    redirect(`/app/onboarding/processes/${processId}/close?${query.toString()}#formalities`);
  }
...
export default async function CloseProcessPage({params,searchParams}:{params:Promise<{processId:string}>,searchParams:Promise<{formalitiesError?:string;summary?:string;recommendations?:string}>}) {
 const access=await requireBOSAccess();
 const {processId}=await params;
 const {formalitiesError,summary="",recommendations=""}=await searchParams;
 ...
 {formalitiesError&&<div className="bos-publish-error" role="alert"><strong>Potwierdź formalności przed decyzją GOTOWY</strong><p>Zaznacz potwierdzenie powyżej. Wpisane podsumowanie i dalsze działania zostały zachowane.</p></div>}
 <label className="bos-close-guided-field"><span>PODSUMOWANIE FAKTÓW</span><small>Zapisz obserwowalne fakty będące podstawą decyzji. Nie opisuj cech osoby.</small><textarea name="summary" required maxLength={1200} defaultValue={summary} placeholder="Np. Wszystkie czynności wykonane i sprawdzone; 2/2 K zaliczone; kryterium kompletacji bez braków potwierdzone w realnej pracy."/></label>
 <label className="bos-close-guided-field"><span>POWÓD / DALSZE DZIAŁANIE</span><small>Wymagane znaczeniowo dla JESZCZE NIE i STOP; przy GOTOWY możesz zapisać dalsze zalecenia operacyjne.</small><textarea name="recommendations" maxLength={1200} defaultValue={recommendations} placeholder="Np. Powtórzyć czynność 04 na kolejnej zmianie i ponownie sprawdzić rezultat."/></label>
 ...
 <div className="bos-close-decisions bos-guided-decisions"><button name="decision" value="READY" disabled={!ready}><strong>GOTOWY</strong><span>{ready?"zakończ wdrożenie pozytywnie":"wymaga kompletnej Bramki Gotowości"}</span></button><button name="decision" value="NOT_YET"><strong>JESZCZE NIE</strong><span>zachowaj postęp i wróć do pracy</span></button><button name="decision" value="STOP"><strong>STOP</strong><span>zakończ bez potwierdzenia gotowości</span></button></div>
 </form></section></>;
}
