...
export default async function CloseProcessPage({params}:{params:Promise<{processId:string}>}) {
 ...
 <label><span>POTWIERDZENIE FORMALNOŚCI</span><span><input type="checkbox" name="formalitiesConfirmed" required/> Potwierdzam, że wymagane formalności dotyczące dopuszczenia do pracy zostały zweryfikowane poza BOS.</span></label>
 ...
 <div className="bos-close-decision-guide"><strong>Wybierz decyzję</strong><p>{ready?"Readiness Gate jest kompletny. System udostępnia decyzję, ale jej nie sugeruje.":"Readiness Gate jest niepełny. Obecna logika produktu blokuje zapis decyzji do czasu spełnienia wszystkich warunków."}</p></div>
 <div className="bos-close-decisions bos-guided-decisions"><button name="decision" value="READY" disabled={!ready}><strong>GOTOWY</strong><span>zakończ wdrożenie pozytywnie</span></button><button name="decision" value="NOT_YET" disabled={!ready}><strong>JESZCZE NIE</strong><span>zachowaj postęp i wróć do pracy</span></button><button name="decision" value="STOP" disabled={!ready}><strong>STOP</strong><span>zakończ bez potwierdzenia gotowości</span></button></div>
 </form></section></>;
}
