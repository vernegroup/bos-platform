...
export async function POST(request: Request) {
 try {
  ...
  const session=await stripe.checkout.sessions.create({
   ...
   metadata:{product:"onboarding",...(access?{organization_id:access.organization.id,bos_user_id:access.user.id}:{})},
   success_url:origin+"/success/onboarding?session_id={CHECKOUT_SESSION_ID}",cancel_url:origin+"/",
  });
