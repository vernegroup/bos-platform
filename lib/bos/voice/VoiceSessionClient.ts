...
export class VoiceSessionClient {
  ...
  start() {
    ...
    this.setSnapshot({
      ...
      hasMicrophone: false, isRealtimeConnected: false, error: null,
    });
  ...
  fail(message: string) {
    this.setSnapshot({ ...this.snapshot, status: "error", isRealtimeConnected: false, error: message });
  }
  ...
  private handleRemoteStream(stream: MediaStream | null) {
    if (!stream || typeof Audio === "undefined") {
