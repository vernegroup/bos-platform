...
const START_MESSAGE: TextChatMessage = {
  ...
  role: "assistant",
  content:
    "Napisz, czego potrzebujesz. Na tym etapie testujemy lokalny przebieg rozmowy; silnik AI zostanie podłączony w kolejnych zadaniach.",
};

const LOCAL_REPLY =
  "Wiadomość została dodana do lokalnej sesji testowej. Po podłączeniu silnika AI w tym miejscu pojawi się właściwa odpowiedź BOS Assistant.";
      ...
      <div className="bos-assistant-body" aria-live="polite" aria-relevant="additions">
        <div className="bos-text-chat-list">
          {messages.map((message) => (
            <div
              className={`bos-assistant-message bos-assistant-message-${message.role}`}
              key={message.id}
            >
        ...
        {messages.length === 1 && (
          <div className="bos-assistant-suggestions" aria-label="Przykładowe pytania">
            <button type="button" onClick={() => sendMessage("Pomóż mi wybrać produkt")}>
              Pomóż mi wybrać produkt
            </button>
            <button type="button" onClick={() => sendMessage("Mam pytanie o wdrożenie")}>
              Mam pytanie o wdrożenie
            </button>
            <button type="button" onClick={() => sendMessage("Jak działa BOS?")}>
              Jak działa BOS?
            </button>
          </div>
        )}
